    ...
    Object.assign(self._barrageTypeHandler, {
      ...
      image: {
        ...
        calcPosition(barrage) {
          ...
          barrage.x -= barrage.offset;
          if (barrage.x + width + SPACING < self._width) {
            // 弹幕完全显示了，并且空出间距了
            if (typeof barrage.index === 'number') self._barrageUsage[barrage.mode][barrage.index] = null;
            barrage.index = null;
          }
          if (barrage.x + width <= 0) {
            // 显示完了，移除
            return true;
          }
          return false;
...
/* harmony export (immutable) */ __webpack_exports__["default"] = Barrage;
